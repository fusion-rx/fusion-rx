...
export function sortDescending<T extends Object>(arr: T[], key: keyof T): T[] {
    if (!isArrayIndexed(arr)) indexArray(arr);
    return sortArray(arr, key, SortDirection.DESCENDING);
}
...
export function sortAscending<T extends Object>(arr: T[], key: keyof T): T[] {
    if (!isArrayIndexed(arr)) indexArray(arr);
    return sortArray(arr, key, SortDirection.ASCENDING);
}
 ...
 * {@link indexArray} or set `indexArr` to true on {@link sortAscending}
 * or {@link sortDescending}.
 */
export function sortClear<T extends Object>(arr: T[]): T[] {
    if (isArrayIndexed(arr)) {
        return arr.sort((a, b) => {
            const aIndex = getArrayElementIndex(a);
            const bIndex = getArrayElementIndex(b);
