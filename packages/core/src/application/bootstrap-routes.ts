...
export const bootstrapRoute = (type: Type<RouteMetadataFacade>) => {
    ...
    const routeInstance = type.prototype.instance;
    const fsnServer = (<FusionServer>(<I>FusionServer).prototype.instance)
        .expressApp;

    if (!fsnServer && !hasLoggedFsnError) {
        GlobalLogger.error(
    ...
    Object.keys(type.prototype.registered ?? {}).forEach((handlerName) => {
        ...
        const handler = routeInstance[handlerName];
        const method = fsnServer[routeMetadata.method];

        if (handler && isFunction(handler)) {
            method(formatRoute(routeMetadata.path), (req: Req, res: Res) =>
                handleRequest(
