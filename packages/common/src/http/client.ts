...
export class HttpClient {
    constructor(private handler: FetchBackend) {}
    ...
    request(
        ...
        // If the primary argument is an instance of `HttpRequest`,
        // The other arguments must be undefined (per the signature)
        // and can be ignored.
        if (first instanceof HttpRequest) {
            req = first;
        } else {
            // Otherwise, `first` represents a string URL.
            // Construct the request, incorporating the remaining arguments
            // assuming `GET` unless a method is provided.

            const headers = this._buildHeaders(options.headers);
            const params = this._buildParams(options.params);
        ...
        return this._selectObserveStream(
            req.responseType,
            res$,
            options.observe
        );
    ...
    private _selectObserveStream = (
        responseType: any,
        res$: Observable<HttpResponse<any>>,
        observe?: any
    ) => {
        switch (observe ?? 'body') {
            case 'body':
                return this._selectBodyObserver(responseType, res$);
            case 'response': // The response stream was requested directly, so return it.
                return res$;
            default: // Guard against new future observe types being added.
                throw new Error(
    ...
    private _selectBodyObserver = (
        ...
        switch (responseType) {
            case 'arraybuffer':
                return this._observeArrayBuffer(res$);
            case 'blob':
                return this._observeBlob(res$);
            case 'text':
                return this._observeText(res$);
            case 'json':
            default:
    ...
    private _observeArrayBuffer = (res$: Observable<HttpResponse<any>>) =>
        res$.pipe(
    ...
    private _observeBlob = (res$: Observable<HttpResponse<any>>) =>
        res$.pipe(
            map((res: HttpResponse<any>) => {
    ...
    private _observeText = (res$: Observable<HttpResponse<any>>) =>
        res$.pipe(
            map((res: HttpResponse<any>) => {
